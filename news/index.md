...
- The shell provides a column-selection panel with char/num type icons
  and a collapse chevron, a property pane (Label, Name, Length, Type,
  Format, Informat), a names-versus-labels header toggle, header
  right-click sort and size-to-content, a free-text row filter, and CSV
...
- The “Show code” toolbar button emits the runnable, air-formatted
  `dplyr` pipeline (select, filter, arrange, with SQL-to-R translation)
  that reproduces the current view.

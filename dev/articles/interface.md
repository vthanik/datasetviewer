...
started](https://vthanik.github.io/datasetviewer/dev/articles/datasetviewer.md)
...
| 2 | **Pin Column** | Freeze this column at the left edge of the grid (see below). |
...
| 7 | **Column details** | Open the per-column statistics dialog (see below). |
| 8 | **Size grid columns to content** | Auto-fit every column width to its contents. |
| 9 | **Restore original column widths** | Reset every column to the default width. |

## Pinning columns and rows

**Pin Column** (header menu) freezes the column at the left edge of the
grid, where it stays put while you scroll horizontally — the anchor for
wide datasets, with the subject identifier pinned while you pan across a
hundred analysis columns. A pinned column keeps working like any other:
it sorts, filters, and exports; a thin accent bar marks it as pinned.
**Unpin Column** in the same menu releases it, and a pinned column that
you hide in the columns panel stays latently pinned — it returns to the
frozen edge when re-shown.

**Pin Row** (right-click a row *number*) snapshots the row into a strip
at the top of the grid, directly under the headers. The strip keeps the
row number the row displayed when you pinned it, so “row 8” still reads
as row 8 even after the grid is re-sorted or filtered above it — pin the
rows you are comparing, then sort and filter freely. Right-click a
pinned row to copy or unpin it.

![](figures/pinning.png)

## Column details

**Column details** (header menu) opens a statistics dialog for the
column, computed by DuckDB over the whole dataset — never a sample.
Every column reports its completeness (valid versus missing) and
unique-value count; a numeric column adds a histogram with mean,
standard deviation, and quartiles, while a categorical column lists its
most frequent values with their shares.

![](figures/column-details.png)
...
started](https://vthanik.github.io/datasetviewer/dev/articles/datasetviewer.html#embedding-in-a-shiny-app)
vignette for a complete app.

## Where to next

- [Get started with
  datasetviewer](https://vthanik.github.io/datasetviewer/dev/articles/datasetviewer.md)

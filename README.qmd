...
- **SAS Studio layout.** A column-selection panel with char/num type icons
  ...
  a names-versus-labels header toggle, click-a-header to sort (Shift-click for
  multi-column sort), and a free-text "Filter Table Rows" expression.
...
Click a column header to sort it (again for descending, once more to clear);
**Shift-click** additional headers to build a multi-column sort, each shown with
its direction and priority (`AGE ↑1`, `SEX ↓2`). Right-click for the header menu
to add a filter, copy the column or its header, or clear the sort. Click the
funnel in the toolbar to edit the filter expression directly (for example
`AGE > 50 and SEX = "M"`).

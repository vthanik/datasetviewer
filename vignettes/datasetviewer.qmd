...
- **Header sort** — click a column header to sort it ascending, again for
  descending, and once more to return to the original order. Shift-click further
  headers to build a multi-column sort; each sorted column shows its direction
  and priority (`AGE ↑1`, `SEX ↓2`).
- **Header menu** — right-click any column header to add a filter, copy the
  column or its header, clear the sort, or size the columns to content.

...
- **Header menu** — right-click any column header to copy or pin the column,
  sort it (Sort Ascending / Descending add the column to the sort; Clear
  Sorting removes just that column), add a filter, open **Column details**
  (per-column statistics: completeness, unique values, a histogram and
  quantiles for numeric columns, top values for categorical ones), or size
  the columns to content.
- **Pinning** — **Pin Column** freezes a column at the left edge so it stays
  put while you scroll horizontally; **Pin Row** (right-click a row number)
  snapshots the row into a strip pinned under the headers, keeping its
  original row number. Both show an accent bar and unpin the same way.
